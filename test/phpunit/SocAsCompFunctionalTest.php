...
/**
 * \file    test/phpunit/SocAsCompFunctionalTest.php
 * \ingroup socascomp
 ...
 *
 * @package Testsocascomp
 */
class SocAsCompFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSocAsComp')->click();
		} else {
			// Disable the module
			$this->byHref('modSocAsComp')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/socascomp/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('socascomp/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/socascomp/admin/about.php');
		$this->authenticate();
		return $this->assertContains('socascomp/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('socascompwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSocAsComp_SocAsCompTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSocAsComp_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
