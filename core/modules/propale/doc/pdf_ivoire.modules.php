...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_ivoire.modules.php
 *	\ingroup    propale
 *	\brief      File of Class to generate PDF proposal with ivoire template
 */
...
/**
 *	Class to generate PDF proposal ivoire
 */
class pdf_ivoire extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "ivoire";
		$this->description = $langs->trans('DocModelIvoireDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (isset($object->array_options["options_socascomp_sender"]) && !empty($object->array_options["options_socascomp_sender"])) {
			$newSoc = new Societe($this->db);
			$newSoc->fetch($object->array_options['options_socascomp_sender']);
			$this->emetteur = $newSoc;
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!getDolGlobalInt('PDF_DISABLE_MYCOMPANY_LOGO')) {
			if ($this->emetteur->logo) {
				...
				if ($this->emetteur !== $mysoc) {
					$logodir = $conf->societe->multidir_output[$conf->entity] . '/' . $this->emetteur->id;
				}
